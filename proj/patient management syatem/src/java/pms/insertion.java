...
public class insertion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            BAL bal=new BAL();
         int suc=-1;
             suc =  bal.insertemp(empid, typeid, fname, lname, deptid, gender, phoneno, email, bloodgroup, username, pass);
         if(suc>0)
         {
         
   request.setAttribute("message1","Employee Added Auccessfully");
...
request.getRequestDispatcher("createemp.jsp").forward(request,response);      
         ...
         out.println("<!DOCTYPE html>");
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet insertion at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
